...
function displayIndexBtn(){
     ...
     for(let i=1; i<=maxIndex; i++){
          pagination.innerHTML += '<button onclick= "paginationBtn('+i+')" home="'+i+'">'+i+'</button>'
     }
...
function highlightIndexBtn(){
     ...
     paginationBtns.forEach(btn => {
          ...
          if(btn.getAttribute('home') === currentIndex.toString()){
     ...
     if(getData.length > 0){
          for(var i=tab_start; i<tab_end; i++){
               ...
               if(staff){
                    ...
                    <td>
                    <button onclick="readInfo('${staff.picture}', '${staff.fName}', '${staff.lName}', 
                                             '${staff.ageVal}', '${staff.cityVal}', 
                                             '${staff.positionVal}', '${staff.salaryVal}', 
                                             '${staff.sDateVal}', '${staff.emailVal}', 
                                             '${staff.phoneVal}')">
                                             <i class="fa-regular fa-eye"></i></button>

                    <button onclick="editInfo('${i}', '${staff.picture}', '${staff.fName}', 
                                             '${staff.lName}', '${staff.ageVal}', 
                                             '${staff.cityVal}', '${staff.positionVal}', 
                                             '${staff.salaryVal}', '${staff.sDateVal}', 
                                             '${staff.emailVal}', '${staff.phoneVal}')">
...
form.addEventListener('submit', (e)=> {
     ...
     const information = {
          id: Date.now(),
          picture: imgInput.src == undefined ? "unknown.png" :imgInput.src,
          fName: fName.value,
          lName: lName.value,
          ageVal: age.value,
          cityVal: city.value,
...
//LOGOUT
     function logout(event){
          ...
          Swal.fire({
               ...
               // Perform the logout action here, such as redirecting to the logout URL
               window.location.href = "welcomee.html";
          }
     });
}
...
function toggleMenu() {
     ...
     if (menu.classList.contains('active')) {
          icon.style.color = 'rgb(0, 255, 255)';
     } else {
          icon.style.color = '#fff';
     }
     
     if (menu.classList.contains('active')) {
          const menuBtnRect = document.querySelector('.menu-btn').getBoundingClientRect();
          menu.style.top = `${menuBtnRect.top + menuBtnRect.height}px`;
